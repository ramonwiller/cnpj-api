"""
ETL para importação da tabela de naturezas jurídicas a partir de CSV.

Arquivo sem cabeçalho (encoding Latin-1, separador ;, aspas duplas):
  "1015";"ÓRGÃO PÚBLICO DO PODER EXECUTIVO FEDERAL"
  "2063";"SOCIEDADE EMPRESÁRIA LIMITADA"
  "2068";"SOCIEDADE ANÔNIMA FECHADA"
"""
...
class NaturezasPipeline(BaseCSVPipeline):
    """Importa naturezas jurídicas a partir de CSV (codigo;descricao), sem linha de cabeçalho."""
    ...
    def transform_row(self, row: dict[str, str]) -> Natureza | None:
        codigo = (row.get("codigo") or "").strip()
        descricao = (row.get("descricao") or "").strip()
        if not codigo or not descricao:
            return None
        return Natureza(codigo=codigo[:7], descricao=descricao)

    async def _persist_one(self, model: Natureza) -> str:
        """Insere ou atualiza uma natureza por código via repository; ignora se já estiver igual (skipped)."""
